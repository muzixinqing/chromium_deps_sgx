...
typedef enum _status_t
{
    ...
    SGX_ERROR_IPLDR_NOTENCRYPTED            = SGX_MK_ERROR(0x8001), /* sgx_create_encrypted_enclave was called but enclave dll is not encrypted */
    SGX_ERROR_IPLDR_MAC_MISMATCH            = SGX_MK_ERROR(0x8002), /* sgx_create_encrypted_enclave was called but there was a verification error when decrypting the data */
    SGX_ERROR_IPLDR_ENCRYPTED               = SGX_MK_ERROR(0x8003), /* sgx_create_enclave was called but enclave dll is encrypted */
} sgx_status_t;
